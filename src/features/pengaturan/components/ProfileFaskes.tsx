...
const ProfileFaskes: React.FC = () => {
  ...
  const reviewBadge = () => {
    ...
    const b = map[d.status_review] || {
      label: d.status_review,
      class: "bg-gray-100 text-gray-600",
    };
    return (
      <span
        className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${b.class}`}
      >
        {b.label}
      </span>
    );
    ...
    <div className="space-y-5 max-w-5xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-dark-bg to-dark-surface rounded-xl overflow-hidden">
        ...
        <div className="px-6 py-3 bg-white/5 flex items-center flex-wrap gap-2 border-t border-white/10">
          <span
            className={`inline-flex items-center space-x-1 rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase ${d.is_aktif === "Iya" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${d.is_aktif === "Iya" ? "bg-green-600" : "bg-red-600"}`}
            />
        ...
        <div className="px-5 pb-4">
          <div className="rounded-lg overflow-hidden border border-surface-border/30 divide-y divide-surface-border/30">
            <InfoRow
              icon={<Building2 className="w-4 h-4" />}
              label="Jenis Faskes"
              value={d.jenis_faskes?.nama}
            />
            <InfoRow
              icon={<ClipboardList className="w-4 h-4" />}
              label="Badan Usaha"
              value={d.jenis_badan_usaha?.nama}
            />
            <InfoRow
              icon={<Hash className="w-4 h-4" />}
              label="Nama Badan Usaha"
              value={d.nama_badan_usaha}
            />
            <InfoRow
              icon={<BadgeCheck className="w-4 h-4" />}
              label="Klinik"
              value={<LabelBadge val={d.is_klinik} />}
            />
            <InfoRow
              icon={<ShieldCheck className="w-4 h-4" />}
              label="Pelanggan Profaskes"
              value={<LabelBadge val={d.is_pelanggan_profaskes} />}
            />
            <InfoRow
              icon={<ClipboardList className="w-4 h-4" />}
              label="Tingkat Pelayanan"
              value={d.tingkat_pelayanan}
            />
          </div>
        </div>
      </div>
          ...
          <div className="px-5 pb-4">
            <div className="rounded-lg overflow-hidden border border-surface-border/30 divide-y divide-surface-border/30">
              <InfoRow
                icon={<User className="w-4 h-4" />}
                label="Nama"
                value={d.nama_penanggung_jawab}
              />
              <InfoRow
                icon={<Mail className="w-4 h-4" />}
                label="Email"
                value={d.email_penanggung_jawab}
              />
              <InfoRow
                icon={<Phone className="w-4 h-4" />}
                label="No. HP"
                value={d.nomor_hp_penanggung_jawab}
              />
            </div>
          </div>
        </div>
        ...
        <div className="px-5 pb-4">
          <div className="rounded-lg overflow-hidden border border-surface-border/30 divide-y divide-surface-border/30">
            <InfoRow
              ...
              label="Alamat"
              value={d.alamat_faskes}
            />
            <InfoRow
              icon={<MapPin className="w-4 h-4" />}
              label="Provinsi"
              value={d.provinsi?.nama_prop}
            />
            <InfoRow
              icon={<MapPin className="w-4 h-4" />}
              label="Kab/Kota"
              value={d.kab_kota?.nama_kab}
            />
            <InfoRow
              icon={<MapPin className="w-4 h-4" />}
              label="Kecamatan"
              value={d.kecamatan?.nama_kec}
            />
          </div>
        </div>
      </div>
      ...
      <div className="bg-surface-muted dark:bg-dark-card rounded-xl border border-surface-border/40 px-5 py-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <span className="text-[10px] text-text-muted flex items-center">
            <Calendar className="w-3 h-3 mr-1" /> Dibuat:{" "}
            {formatDateTime(d.created_at)}
          </span>
          <span className="text-[10px] text-text-muted flex items-center">
            <Calendar className="w-3 h-3 mr-1" /> Diperbarui:{" "}
